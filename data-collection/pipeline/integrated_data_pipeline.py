...
class IntegratedDataPipeline:
    ...
    def run_pipeline(self) -> PipelineStats:
        ...
        logger.info(
            f"Processed {self.stats.total_raw_events} → {self.stats.final_unique_events} events"
        )
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            ...
            if self.eventbrite_scraper:
                future = executor.submit(
                    self._collect_eventbrite_events, start_date, end_date
                )
                futures.append(("eventbrite", future))

            # Meetup
            future = executor.submit(self._collect_meetup_events, start_date, end_date)
    ...
    def _collect_eventbrite_events(
        ...
        for event in events:
            standardized = {
                ...
                "popularity_score": min(
                    1.0, event.attendee_count / 100.0
    ...
    def _collect_tiktok_events(self) -> List[Dict]:
        ...
        for event in events:
            standardized = {
                ...
                "source": "tiktok",
                "popularity_score": min(
                    1.0, (event.views + event.likes * 10) / 100000.0
                ),  # Weighted popularity
                "artists": event.detected_artists,
                "genres": event.detected_genres,
            }
    ...
    def _collect_viral_events(self) -> List[Dict]:
        ...
        for event in trending_events:
            ...
            primary_signal = max(
                event.platform_signals, key=lambda s: s.engagement_score
            )
            ...
            try:
                ...
                if result.status == ValidationStatus.VALID:
                    ...
                    if result.confidence_score >= self.config.min_confidence_score:
                        ...
                        logger.warning(
                            f"Rejected suspicious event: {event.get('title', 'Unknown')}"
                        )
                    ...
                    self.stats.validation_failed += 1
                    logger.warning(
                        f"Rejected invalid event: {event.get('title', 'Unknown')}"
                    )

            except Exception as e:
                logger.error(
                    f"Validation failed for event {event.get('id', 'unknown')}: {e}"
                )
                self.stats.validation_failed += 1

        logger.info(
            f"Validation: {self.stats.validation_passed} passed, {self.stats.validation_failed} failed"
        )
        ...
        for event in events:
            try:
                ...
                if result.enriched_artists:
                    ...
                    event["artists"] = [
                        artist.canonical_name for artist in result.enriched_artists
                    ]
                    ...
                    if result.enriched_artists:
                        avg_popularity = sum(
                            a.popularity_score for a in result.enriched_artists
                        ) / len(result.enriched_artists)
                        event["popularity_score"] = max(
                            event.get("popularity_score", 0), avg_popularity
                        )
                ...
                logger.warning(
                    f"Enrichment failed for event {event.get('id', 'unknown')}: {e}"
                )
                enriched_events.append(event)  # Keep original event

        self.stats.enrichment_successful = successful_enrichments
        self.stats.enrichment_coverage = (
            successful_enrichments / len(events) if events else 0
        )

        logger.info(
            f"Enrichment: {successful_enrichments}/{len(events)} events enriched"
        )
    ...
    def _deduplicate_events(self, events: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            self.stats.duplicate_detection_accuracy = result.confidence_scores.get(
                "overall_confidence", 0.0
            )

            logger.info(
                f"Deduplication: Removed {result.duplicates_found} duplicates in {len(result.duplicate_groups)} groups"
            )
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    logger.warning(
                        f"Failed to store event {event.get('id', 'unknown')}: {e}"
                    )
    ...
    def _generate_report(self):
        ...
        report = {
            ...
            "quality_metrics": {
                ...
                "data_retention_rate": (
                    self.stats.final_unique_events / self.stats.total_raw_events
                    if self.stats.total_raw_events > 0
                    else 0
                ),
            },
        }
    ...
    def _save_raw_data(self, events: List[Dict]):
        ...
        output_path = (
            Path(self.config.output_dir)
            / f"raw_events_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _save_processed_data(self, events: List[Dict]):
        ...
        output_path = (
            Path(self.config.output_dir)
            / f"processed_events_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        with open(output_path, "w") as f:
            json.dump(events, f, indent=2, default=str)
...
def main():
    ...
    try:
        ...
        print(
            f"Data retention rate: {(stats.final_unique_events/stats.total_raw_events*100) if stats.total_raw_events > 0 else 0:.1f}%"
        )
