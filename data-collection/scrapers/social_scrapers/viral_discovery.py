...
class ViralEventDiscoveryEngine:
    ...
    def discover_viral_events(
        self, days_back: int = 3, min_viral_score: float = 0.6, max_events: int = 50
    ) -> List[TrendingEvent]:
        """
        ...
        viral_events = [
            event for event in trending_events if event.viral_score >= min_viral_score
        ]
    ...
    def _collect_tiktok_signals(self, days_back: int) -> List[ViralEventSignal]:
        ...
        try:
            ...
            tiktok_events = self.tiktok_scraper.scrape_venues(
                days_back=days_back, max_videos_per_venue=30
            )
    ...
    def _collect_instagram_signals(self, days_back: int) -> List[ViralEventSignal]:
        ...
        try:
            ...
            instagram_events = self.instagram_scraper.scrape_viral_events(
                days_back=days_back, max_posts_per_hashtag=30, min_engagement=100
            )
    ...
    def _aggregate_signals_to_events(
        self, signals: List[ViralEventSignal]
    ) -> List[TrendingEvent]:
    ...
    def _create_trending_event(
        self, event_key: str, signals: List[ViralEventSignal]
    ) -> TrendingEvent:
    ...
    def _calculate_viral_score(self, event: TrendingEvent) -> float:
        ...
        # Combine scores
        viral_score = (
            engagement_score + platform_bonus + indicator_score + hashtag_score
        )
    ...
    def _calculate_confidence_score(self, event: TrendingEvent) -> float:
        ...
        now = datetime.now()
        recent_signals = [
            s for s in event.platform_signals if (now - s.timestamp).hours <= 24
        ]
    ...
    def analyze_viral_patterns(self, events: List[TrendingEvent]) -> Dict[str, any]:
        ...
        analysis = {
            ...
            "avg_viral_score": (
                sum(e.viral_score for e in events) / len(events) if events else 0
            ),
            "platform_distribution": Counter(
                signal.platform for event in events for signal in event.platform_signals
            ),
            ...
            "trending_hashtags": Counter(
                hashtag for event in events for hashtag in event.trending_hashtags
            ).most_common(20),
        }
...
def main():
    ...
    viral_events = engine.discover_viral_events(
        days_back=3, min_viral_score=0.5, max_events=20
    )
    ...
    for i, event in enumerate(viral_events[:5], 1):
        ...
        print(
            f"   Platforms: {', '.join(set(s.platform for s in event.platform_signals))}"
        )
