...
class InstagramViralEventScraper(InstagramEventScraper):
    ...
    VIRAL_HASHTAGS = [
        ...
        'copenhagennow', 'cphweekend', 'copenhagenweekend', 'copenhagen_events',
        'cphlife', 'copenhagenlife', 'visitcopenhagen', 'cphvibes',
        
        # Underground/secret events
        'undergroundcph', 'secretpartycopenhagen', 'hiddencph', 'cphunderground',
        'warehousepartycopenhagen', 'rooftoppartycopenhagen', 'secretlocation',
        'popupparty', 'undergroundrave', 'secretrave', 'invite_only',
        
        # Trending/viral discovery
        'copenhagennightlife', 'cphparty', 'copenhagenparty', 'nightoutcopenhagen',
        'cphnight', 'copenhagennight', 'partytime', 'nightlife', 'clubbing',
        'ravecph', 'technikcph', 'housemusiccph', 'electronicmusiccph',
        
        # Neighborhood-specific viral content
        'vesterbrovibes', 'vesterbronights', 'nørrebrovibes', 'nørrebronights',
        'indrebynight', 'christianiavibes', 'østerbronights', 'islandsbrygge',
        'refshaleøen', 'papirøen', 'sydhavnen', 'nordhavn',
        
        # Event type hashtags that go viral
        'technocopenhagen', 'housecopenhagen', 'jazzcopenhagen', 'rockcopenhagen',
        'indiecopenhagen', 'festivaler', 'koncerter', 'dj_set', 'livemusic',
        'alternativecopenhagen', 'experimentalmusic', 'clubnight'
    ]
    
    # User types that often post viral event content (not venues)
    VIRAL_USER_TYPES = [
        'event_photographer', 'party_photographer', 'nightlife_blogger',
        'music_blogger', 'copenhagen_influencer', 'party_goer', 'raver',
        'event_hunter', 'underground_scout', 'music_lover', 'scene_reporter'
    ]
        ...
        try:
            for post in hashtag_obj.get_posts():
                ...
                if self._is_viral_event_post(post):
                    ...
                    event = self._extract_viral_event_data(post, venue_name, username, hashtag_name)
                    if event:
                        events.append(event)
    ...
    def _scrape_viral_users(
        ...
        viral_usernames = [
            ...
            'copenhagen_nightlife_scout', 'cph_underground', 'party_hunter_cph',
            'copenhagen_raves', 'cph_events_insider', 'nightlife_copenhagen',
            'underground_cph', 'techno_copenhagen', 'house_copenhagen'
        ]
    ...
    def _is_viral_event_post(self, post) -> bool:
        ...
        hashtags = [tag.lower() for tag in post.caption_hashtags] if post.caption_hashtags else []
        
        # Viral event indicators
        viral_indicators = [
            'found this party', 'stumbled upon', 'discovered', 'hidden gem',
            'secret location', 'underground', 'word of mouth', 'invite only',
            'crazy night', 'insane party', 'best night ever', 'you need to go',
            'dont miss', 'trending', 'viral', 'everyone talking about',
            'packed', 'sold out', 'queue around the block', 'legendary',
            'spontaneous', 'popup', 'last minute', 'surprise event',
            'hemmelig location', 'alle snakker om', 'gå ikke glip af'
        ]
        
        # Event urgency indicators
        urgency_indicators = [
            'tonight', 'right now', 'happening now', 'live', 'currently',
            'i aften', 'lige nu', 'sker nu', 'this weekend', 'denne weekend'
        ]
        
        # Copenhagen location indicators
        location_indicators = [
            'copenhagen', 'cph', 'københavn', 'vesterbro', 'nørrebro',
            'indre by', 'christiania', 'østerbro', 'islands brygge',
            'refshaleøen', 'papirøen', 'sydhavnen', 'nordhavn'
        ]
        ...
        viral_hashtag_patterns = [
            'underground', 'secret', 'popup', 'warehouse', 'rooftop', 'rave',
            'tonight', 'weekend', 'viral', 'trending', 'epic', 'insane'
        ]
    ...
    def _extract_venue_from_viral_post(self, post) -> str:
        ...
        neighborhoods = [
            'Vesterbro', 'Nørrebro', 'Indre By', 'Christiania', 
            'Østerbro', 'Islands Brygge', 'Refshaleøen', 'Nordhavn'
        ]
