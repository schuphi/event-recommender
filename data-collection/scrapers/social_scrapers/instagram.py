...
class InstagramEventScraper:
    ...
    EVENT_KEYWORDS = {
        'english': [
            # Basic event types
            'concert', 'live', 'show', 'event', 'party', 'night', 'dj', 'performance',
            # Viral/trending language
            'underground', 'secret', 'popup', 'warehouse', 'rooftop', 'rave', 'festival',
            'tonight', 'happening', 'dont miss', 'viral', 'trending', 'epic', 'insane',
            'legendary', 'packed', 'sold out', 'queue', 'hidden gem', 'discovered',
            # Engagement language
            'everyone talking', 'word of mouth', 'invite only', 'last minute',
            'spontaneous', 'surprise', 'crazy night', 'best party', 'unreal'
        ],
        'danish': [
            # Basic Danish event terms
            'koncert', 'optræden', 'fest', 'aften', 'nat', 'live', 'show',
            # Viral Danish terms
            'hemmelig', 'underground', 'popup', 'lager', 'tag', 'rave',
            'i aften', 'sker nu', 'gå glip', 'viral', 'trending', 'episk',
            'legendarisk', 'proppet', 'udsolgt', 'kø', 'skjult perle',
            'alle snakker', 'mund til mund', 'kun inviterede', 'sidste øjeblik'
        ]
    }
    
    GENRE_KEYWORDS = [
        'techno', 'house', 'electronic', 'disco', 'funk', 'soul', 'jazz', 'blues',
        'rock', 'punk', 'indie', 'alternative', 'pop', 'hip hop', 'rap', 'r&b',
        'ambient', 'experimental', 'classical', 'folk', 'acoustic', 'reggae'
    ]
    ...
    def _is_event_post(self, post) -> bool:
        ...
        date_patterns_found = any(
            re.search(pattern, caption, re.IGNORECASE)
        ...
        event_hashtags = any(
            any(keyword in tag for keyword in self.EVENT_KEYWORDS['english'] + self.EVENT_KEYWORDS['danish'])
            for tag in hashtags
        )
    ...
    def _extract_artists_from_text(self, text: str) -> List[str]:
        ...
        common_words = {
            'Instagram', 'Copenhagen', 'Denmark', 'Friday', 'Saturday', 'Sunday',
            'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Tonight', 'Tomorrow',
            'Event', 'Concert', 'Live', 'Music', 'Dance', 'Party'
        }
