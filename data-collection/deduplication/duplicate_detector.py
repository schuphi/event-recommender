...
class EventDuplicateDetector:
    ...
    def detect_duplicates(self, events: List[Dict]) -> DeduplicationResult:
        ...
        logger.info(
            f"Found {duplicates_found} duplicates in {len(duplicate_groups)} groups"
        )
    ...
    def _preprocess_events(self, events: List[Dict]) -> List[Dict]:
        ...
        for i, event in enumerate(events):
            ...
            processed_event["_title_normalized"] = self._normalize_text(
                event.get("title", "")
            )
            processed_event["_description_normalized"] = self._normalize_text(
                event.get("description", "")
            )
            processed_event["_venue_normalized"] = self._normalize_venue_name(
        ...
        for i in range(len(cluster)):
            for j in range(i + 1, len(cluster)):
                ...
                if similarity_result["total_score"] >= self.thresholds["possible"]:
                    match_type = self._classify_match_confidence(
                        similarity_result["total_score"]
                    )
    ...
    def _find_cross_cluster_duplicates(
        ...
        for i in range(len(events)):
            for j in range(i + 1, len(events)):
                ...
                title_sim = self._text_similarity(
                    event1["_title_normalized"], event2["_title_normalized"]
                )
    ...
    def _calculate_event_similarity(self, event1: Dict, event2: Dict) -> Dict[str, any]:
        ...
        components["venue_similarity"] = self._venue_similarity(
            event1["_venue_normalized"], event2["_venue_normalized"]
        )

        # Time similarity
        components["time_similarity"] = self._time_similarity(
            event1.get("_start_time_normalized"), event2.get("_start_time_normalized")
        )

        # Description similarity
        components["description_similarity"] = self._text_similarity(
            event1["_description_normalized"], event2["_description_normalized"]
        )

        # Location similarity
        components["location_similarity"] = self._location_similarity(
            event1.get("_location"), event2.get("_location")
        )

        # Artist similarity
        components["artist_similarity"] = self._artist_similarity(
            event1["_artists_normalized"], event2["_artists_normalized"]
        )
        ...
        return {"total_score": total_score, "components": components}
    ...
    def _time_similarity(
        self, time1: Optional[datetime], time2: Optional[datetime]
    ) -> float:
    ...
    def _location_similarity(
        self, loc1: Optional[Tuple], loc2: Optional[Tuple]
    ) -> float:
    ...
    def _calculate_confidence_scores(
        ...
        high_confidence_matches = sum(
            1 for m in matches if m.match_type in ["exact", "high_confidence"]
        )
        avg_similarity = sum(m.similarity_score for m in matches) / total_matches

        # Overall confidence based on match quality
        overall_confidence = (
            high_confidence_matches / total_matches
        ) * 0.6 + avg_similarity * 0.4

        return {
            "overall_confidence": overall_confidence,
            "avg_similarity": avg_similarity,
            "high_confidence_ratio": (
                high_confidence_matches / total_matches if total_matches > 0 else 0.0
            ),
