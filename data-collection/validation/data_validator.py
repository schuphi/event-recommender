...
class EventDataValidator:
    ...
    def validate_event(self, raw_data: Dict) -> ValidationResult:
        ...
        try:
            ...
            confidence_score = max(
                0.0, 1.0 - (error_count * 0.3) - (warning_count * 0.1)
            )
            ...
            return ValidationResult(
                ...
                issues=[
                    ValidationIssue(
                        "system", "error", f"Validation system error: {str(e)}"
                    )
                ],
    ...
    def _validate_geography(self, data: Dict) -> List[ValidationIssue]:
        ...
        if has_lat and has_lon:
            try:
                ...
                distance_to_copenhagen = geodesic(
                    event_location, self.copenhagen_center
                ).kilometers
    ...
    def _clean_data(self, data: Dict) -> Dict:
        ...
        field_mappings = {
            ...
            "image_url": lambda x: (
                str(x).strip() if x and self._is_valid_url(x) else None
            ),
            "artists": lambda x: (
                [s.strip() for s in x if isinstance(s, str) and s.strip()]
                ...
                if isinstance(data[field], datetime):
                    ...
                    try:
                        ...
                        if field == "start_time":  # Required field
                            cleaned[field] = datetime.now() + timedelta(
                                hours=24
                            )  # Default to tomorrow
    ...
    def _geocode_address(self, address: str) -> Optional[Dict]:
        ...
        try:
            ...
            location = self.geocoder.geocode(full_address, timeout=10, exactly_one=True)
