...
class ArtistGenreEnricher:
    ...
    def __init__(
        ...
        self.session.headers.update({"User-Agent": "Copenhagen-Event-Recommender/1.0"})
    ...
    def enrich_event_artists(
        self, event_data: Dict, deep_enrichment: bool = True
    ) -> EnrichmentResult:
        """
    ...
    def _extract_artists_from_text(self, event_data: Dict) -> List[str]:
        ...
        text_sources = [event_data.get("title", ""), event_data.get("description", "")]
    ...
    def _enrich_with_spotify(self, artist_name: str) -> Optional[ArtistInfo]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"Spotify API error for {artist_name}: {response.status_code}"
                )
                return None
    ...
    def _enrich_with_lastfm(self, artist_name: str) -> Optional[ArtistInfo]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"Last.fm API error for {artist_name}: {response.status_code}"
                )
                return None
    ...
    def _enhance_genres(
        self, enriched_artists: List[ArtistInfo], event_data: Dict
    ) -> List[str]:
        ...
        enhanced_genres = self.genre_classifier.clean_and_standardize_genres(
            enhanced_genres
        )
    ...
    def _calculate_enrichment_confidence(
        self, artists: List[ArtistInfo], sources: Set[str]
    ) -> float:
        ...
        # Artist data quality
        avg_popularity = sum(artist.popularity_score for artist in artists) / len(
            artists
        )
    ...
    def _ensure_spotify_token(self) -> bool:
        ...
        if (
            self.spotify_token
            and self.spotify_token_expires
            and datetime.now() < self.spotify_token_expires
        ):
            return True
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.spotify_token_expires = datetime.now() + timedelta(
...
class GenreClassifier:
    ...
    def infer_genres_from_name(self, artist_name: str) -> List[str]:
        ...
        if any(
            ...
            for indicator in ["kollektiv", "møller", "æ", "ø", "å"]
        ):
