...
try:
    ...
    logging.warning(
        "Transformers not available, falling back to regex-based extraction"
    )
...
class EventNLPExtractor:
    ...
    def _compile_date_patterns(self) -> List[Tuple[re.Pattern, str]]:
        ...
        patterns = [
            ...
            (
                re.compile(
                    r"\b(mandag|tirsdag|onsdag|torsdag|fredag|lørdag|søndag)\b",
                    re.IGNORECASE,
                ),
                "weekday_da",
            ),
            ...
            (
                re.compile(r"\b(denne weekend|this weekend)\b", re.IGNORECASE),
                "this_weekend",
            ),
            # Times
...
def main():
    ...
    for date in dates:
        print(
            f"- {date.datetime} (confidence: {date.confidence:.2f}) - '{date.original_text}'"
        )
