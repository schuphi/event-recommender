...
def check_test_dependencies():
    ...
    if missing_packages:
        print(f"❌ Missing test dependencies:")
    ...
    if coverage_json.exists():
        try:
            with open(coverage_json, 'r') as f:
                ...
                summary['total_coverage'] = coverage_data.get('totals', {}).get('percent_covered', 0)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Copenhagen Event Recommender Test Runner")
    parser.add_argument('command', nargs='?', default='all',
                       choices=['all', 'quick', 'api', 'ml', 'data', 'database', 
                               'performance', 'security', 'deps'],
