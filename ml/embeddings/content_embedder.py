...
class ContentEmbedder:
    ...
    def _load_models(self):
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                model_path = self.cache_dir / f"sentence_model_{self.model_name.replace('/', '_')}"
    ...
    def _create_event_features(self, event: Dict, embedding: np.ndarray) -> EventFeatures:
        ...
        if weights['genre'] > 0:
            ...
            for i, cf in enumerate(candidate_features):
                ...
                if query_genres and candidate_genres:
                    genre_sim = len(query_genres & candidate_genres) / len(query_genres | candidate_genres)
