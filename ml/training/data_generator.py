...
class TrainingDataGenerator:
    ...
    def generate_training_data(
        ...
        if len(interactions) < 100:
            logger.warning(f"Only {len(interactions)} interactions available. Need more data.")
            return [], []
        ...
        logger.info(f"Positive examples: {sum(1 for ex in all_examples if ex.label == 1)}")
        logger.info(f"Negative examples: {sum(1 for ex in all_examples if ex.label == 0)}")
    ...
    def _load_data(self) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        if (self._events_cache is not None and 
            ...
            self._interactions_cache is not None):
        ...
        logger.info(f"Loaded {len(events)} events, {len(users)} users, {len(interactions)} interactions")
    ...
    def _generate_positive_examples(
        ...
        for interaction in interactions:
            ...
            if interaction.get('rating') and interaction['rating'] >= self.positive_rating_threshold:
                is_positive = True
            ...
            try:
                ...
                logger.warning(f"Failed to create positive example for {user_id}, {event_id}: {e}")
                continue
        ...
        for user_id in valid_users:
            ...
            for event_id in sampled_events:
                ...
                try:
                    ...
                    logger.warning(f"Failed to create negative example for {user_id}, {event_id}: {e}")
                    continue
    ...
    def _balance_examples_by_user(
        ...
        target_per_user = max(10, total_examples // (len(valid_users) * 2))  # At least 10, reasonable cap
        ...
        for user_id in valid_users:
            ...
            if len(user_exs) <= target_per_user:
                ...
                sampled_pos = random.sample(positive_exs, pos_target) if positive_exs else []
                sampled_neg = random.sample(negative_exs, min(len(negative_exs), neg_target)) if negative_exs else []
                
                balanced_examples.extend(sampled_pos + sampled_neg)
        
        logger.info(f"Balanced from {len(examples)} to {len(balanced_examples)} examples")
...
def main():
    ...
    print(f"Positive ratio: {sum(ex.label for ex in train_examples) / len(train_examples):.2f}")
