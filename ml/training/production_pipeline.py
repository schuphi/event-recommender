...
class ProductionMLPipeline:
    ...
    def __init__(
        ...
        self.data_generator = TrainingDataGenerator(
            db_path=db_path, output_dir=str(data_dir)
        )
    ...
    def run_full_pipeline(
        ...
        try:
            ...
            if len(interactions) < 50:
                logger.warning(
                    f"Only {len(interactions)} interactions found. May affect model quality."
                )
            ...
            if retrain_all or not self._content_model_exists():
                ...
                content_model = self.model_trainer.train_content_model(
                    events, interactions
                )
            ...
            if len(interactions) >= 100:  # Need sufficient interactions
                if retrain_all or not self._cf_model_exists():
                    ...
                    logger.info(
                        "Step 3: Loading existing collaborative filtering model..."
                    )
                ...
                logger.info(
                    "Step 3: Skipping collaborative filtering (insufficient interactions)"
                )
            ...
            if retrain_all or not self._training_data_exists():
                ...
                train_examples, val_examples = (
                    self.data_generator.generate_training_data(
                        max_examples=max_training_examples, balance_users=True
                    )
                )
                ...
                results["training_data_stats"] = {
                    ...
                    "positive_ratio": sum(ex.label for ex in train_examples)
                    / len(train_examples),
                }
            ...
            if retrain_all or not self._hybrid_model_exists():
                ...
                if tune_hyperparameters:
                    ...
                    hybrid_results = self.hybrid_trainer.train(
                        train_examples, val_examples
                    )
            ...
            logger.info(
                f"Production ML pipeline completed in {results['total_duration_minutes']:.1f} minutes"
            )
            ...
            logger.error(
                f"Pipeline failed at step {len(results['steps_completed']) + 1}: {e}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Production ML Pipeline for Event Recommender"
    )
    parser.add_argument(
        "--db-path", default="data/events.duckdb", help="Path to DuckDB database"
    )
    parser.add_argument(
        "--models-dir", default="ml/models", help="Directory for model storage"
    )
    parser.add_argument(
        "--retrain-all", action="store_true", help="Retrain all models from scratch"
    )
    parser.add_argument(
    ...
    # Initialize pipeline
    pipeline = ProductionMLPipeline(db_path=args.db_path, models_dir=args.models_dir)
