...
class HybridRankerTrainer:
    ...
    def __init__(
        self, model_dir: str = "ml/models/hybrid", device: Optional[str] = None
    ):
        """
    ...
    def train(
        ...
        self.model = HybridRankerMLP(
            input_dim=8, hidden_dims=config["hidden_dims"], dropout=config["dropout"]
        ).to(self.device)
        ...
        for epoch in range(config["num_epochs"]):
            ...
            # Validation phase
            val_loss, val_auc, val_metrics = self._validate_epoch(val_loader, criterion)
    ...
    def _validate_epoch(
        ...
        if len(set(all_labels)) > 1:
            ...
            metrics = {"pr_auc": pr_auc, "accuracy": accuracy}
        else:
            auc_score = 0.5
    ...
    def _final_evaluation(
        ...
        if len(set(all_labels)) > 1:
            ...
            metrics["precision"] = (
                ((predictions_binary == 1) & (np.array(all_labels) == 1)).sum()
                / (predictions_binary == 1).sum()
                if (predictions_binary == 1).sum() > 0
                else 0
            )
            metrics["recall"] = (
                ((predictions_binary == 1) & (np.array(all_labels) == 1)).sum()
                / (np.array(all_labels) == 1).sum()
                if (np.array(all_labels) == 1).sum() > 0
                else 0
            )
        ...
        for user_id, user_exs in user_examples.items():
            ...
            user_indices = [
                i for i, ex in enumerate(val_examples) if ex.user_id == user_id
            ]
...
def main():
    ...
    logger.info(
        f"Loaded {len(train_examples)} train and {len(val_examples)} validation examples"
    )
