...
class FeatureEngineer:
    ...
    def extract_price_features(
        ...
        if user_budget_min is not None and user_budget_max is not None:
            ...
            overlap = max(0, min(price_max, user_budget_max) - max(price_min, user_budget_min))
    ...
    def extract_content_features(
        ...
        common_genres = [
            'techno', 'house', 'electronic', 'indie', 'rock', 'jazz', 'pop'
        ]
    ...
    def extract_diversity_features(
        ...
        recommended_venues = {
            rec_event.get('venue_name', '').lower() 
            for rec_event in recommended_events
        }
    ...
    def normalize_features(
        ...
        for feature_name, value in features.items():
            if feature_name in normalization_configs:
                ...
                if config.get('log'):
                    normalized[feature_name] = np.log(max(1, value)) / 10.0  # Divide by 10 for scaling
                elif 'max' in config:
...
def main():
    ...
    # Sample event data
    event_datetime = datetime.now() + timedelta(days=5, hours=3)  # 5 days from now, 3 hours later
