...
class DatabaseService:
    ...
    def __init__(self, config: Optional[DatabaseConfig] = None):
        ...
        if settings.DATABASE_URL and not settings.DATABASE_URL.startswith('../../'):
            ...
            from pathlib import Path
            self.config.db_path = str(Path(__file__).parent.parent.parent.parent / "data" / "events.duckdb")
    ...
    async def _execute_query(self, query: str, params: Optional[List] = None) -> List[Tuple]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    logger.error(f"Query failed after {self.config.max_retries} attempts: {query[:100]}")
                    raise
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if params:
                    ...
                    await loop.run_in_executor(
                        None, lambda: conn.execute(query)
                    )
                return
                ...
                if attempt == self.config.max_retries - 1:
                    logger.error(f"Command failed after {self.config.max_retries} attempts: {query[:100]}")
                    raise
    ...
    async def create_user(
        ...
        """, [
            ...
            location_lat, location_lon, h3_index
        ])
    ...
    def _build_event_filters(
        ...
        if filters.get('genres'):
            ...
            for genre in filters['genres']:
                genre_conditions.append("EXISTS (SELECT 1 FROM event_artists ea JOIN artists a ON ea.artist_id = a.id WHERE ea.event_id = e.id AND JSON_EXTRACT(a.genres, '$') LIKE ?)")
    ...
    async def get_event(self, event_id: str) -> Optional[EventResponse]:
        ...
        events = await self.get_events(limit=1, offset=0)  # This will need a filter by ID
        ...
        if artist_ids:
            ...
            """.format(','.join('?' * len(artist_ids))), artist_ids)
    ...
    async def record_interaction(
        ...
        if update_result[0][0] > 0:
            ...
            next_id_result = await self._execute_query("SELECT COALESCE(MAX(id), 0) + 1 FROM interactions")
            ...
            await self._execute_command("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, [next_id, user_id, event_id, interaction_type, rating, source, position])
    ...
    async def get_user_interactions(
        ...
        result = await self._execute_query(f"""
