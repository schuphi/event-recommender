...
class SingleUserCollaborativeFilter:
    ...
    def learn_from_interactions(
        ...
        """
        logger.info(f"Learning collaborative patterns from {len(user_interactions)} interactions")
    ...
    def _learn_event_cooccurrence(
        ...
        event_lookup: Dict[str, EventResponse]
    ) -> None:
        ...
        for window_interactions in time_windows:
            liked_events = [
                ...
                if interaction.get('interaction_type') in ['like', 'going', 'went'] or
                   (interaction.get('rating', 0) >= 4)
            ]
    ...
    def _learn_context_patterns(
        ...
        for interaction in interactions:
            ...
            contexts = self._identify_interaction_contexts(event, timestamp, interaction)
                    ...
                    for j, event_id_2 in enumerate(event_ids):
                        if i != j:
                            ...
                            if similarity_score > 0.1:  # Only store significant similarities
                                self.event_feature_similarity[(event_id_1, event_id_2)] = similarity_score
    ...
    def get_collaborative_recommendations(
        ...
        for candidate in candidate_events:
            ...
            if cooccur_score > 0:
                ...
                reasons.append(f"Often enjoyed with your liked events ({cooccur_score:.2f})")
            ...
            if feature_score > 0:
                ...
                reasons.append(f"Similar content to your preferences ({feature_score:.2f})")
            ...
            if context_score > 0:
                ...
                reasons.append(f"Matches your behavioral patterns ({context_score:.2f})")
            ...
            if evolution_score > 0:
                ...
                reasons.append(f"Aligns with your taste evolution ({evolution_score:.2f})")
    ...
    def _calculate_cooccurrence_score(self, event_id: str, seed_events: List[str]) -> float:
    ...
    def _calculate_feature_similarity_score(self, event_id: str, seed_events: List[str]) -> float:
            ...
            if context in self.user_context_segments:
                ...
                if context_events:
                    ...
                    positive_interactions = sum(
                        1 for interaction in context_events
                        if interaction.get('interaction_type') in ['like', 'going', 'went'] or
                           interaction.get('rating', 0) >= 4
                    )
    ...
    def _calculate_evolution_score(self, candidate: EventResponse, seed_events: List[str]) -> float:
        ...
        for genre in candidate.genres:
            ...
            for transition, strength in self.preference_evolution['genre_transitions'].items():
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            'cooccurrence_pairs': sum(
                len(event_dict) for event_dict in self.event_cooccurrence_matrix.values()
            ),
            ...
            'venue_exploration_events': len(self.preference_evolution['venue_exploration_pattern'])
        }
