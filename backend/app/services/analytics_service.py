...
class AnalyticsService:
    ...
    async def _write_event_to_db(self, db: 'DatabaseService', event: AnalyticsEvent) -> None:
    ...
    async def _load_existing_metrics(self) -> None:
        ...
        try:
            ...
            interactions_result = await db._execute_query("SELECT COUNT(*) FROM interactions")
            ...
            recommendations_result = await db._execute_query("SELECT COUNT(*) FROM recommendation_logs")
    ...
    def _track_event(self, event_type: str, user_id: Optional[str] = None, **data) -> None:
    ...
    async def get_dashboard_data(self, days: int = 7) -> AnalyticsResponse:
        ...
        try:
                ...
                if row[0]:
                    try:
                        ...
                        for genre in genre_list:
                            events_by_genre[genre] = events_by_genre.get(genre, 0) + row[1]
                    except:
                        pass
            ...
            events_by_neighborhood = {row[0]: row[1] for row in neighborhoods_result if row[0]}
            ...
            interaction_breakdown = {row[0]: row[1] for row in interaction_breakdown_result}
