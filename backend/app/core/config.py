...
class Settings(BaseSettings):
    ...
    CONTENT_MODEL_DIR: str = Field(
        default="ml/models/content_based", env="CONTENT_MODEL_DIR"
    )
    ...
    DEFAULT_RECOMMENDATION_COUNT: int = Field(
        default=10, env="DEFAULT_RECOMMENDATION_COUNT"
    )
    ...
    # Model Training
    AUTO_RETRAIN_HOURS: int = Field(
        default=24, env="AUTO_RETRAIN_HOURS"
    ...
    SCRAPING_SCHEDULE_CRON: str = Field(
        default="0 2 * * *", env="SCRAPING_SCHEDULE_CRON"
    )  # Daily at 2 AM
    MAX_EVENTS_PER_SCRAPER: int = Field(default=1000, env="MAX_EVENTS_PER_SCRAPER")

    # Feature Flags
    ENABLE_COLLABORATIVE_FILTERING: bool = Field(
        default=True, env="ENABLE_COLLABORATIVE_FILTERING"
    )
    ...
    ENABLE_COLD_START_ONBOARDING: bool = Field(
        default=True, env="ENABLE_COLD_START_ONBOARDING"
    )
    ...
    RECOMMENDATION_TIMEOUT_SECONDS: int = Field(
        default=10, env="RECOMMENDATION_TIMEOUT_SECONDS"
    )
