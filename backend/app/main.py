    ...
    try:
        ...
        if background_tasks:
            background_tasks.add_task(
                analytics_service.track_user_registration, user_id=request.user_id
            )
...
async def update_user_preferences(
    ...
    try:
        ...
        if background_tasks:
            background_tasks.add_task(
                analytics_service.track_preference_update, user_id=user_id
            )
...
async def get_event(event_id: str, db: DatabaseService = Depends(get_db_service)):
...
async def get_similar_events(
    ...
    try:
        similar_events = await recommendation_service.get_similar_events(
            event_id=event_id, num_recommendations=num_recommendations
        )
...
async def get_user_interactions(
    ...
    try:
        interactions = await db.get_user_interactions(
            user_id=user_id, limit=limit, interaction_type=interaction_type
        )
...
async def search_events(
    ...
    try:
        ...
        return {"query": request.query, "events": events, "count": len(events)}
...
async def get_analytics_dashboard(
    days: int = 7, admin_user: str = Depends(get_current_user)  # Require authentication
):
...
async def get_event_analytics(event_id: str, days: int = 30):
    """Get analytics for a specific event."""

    try:
        analytics = await analytics_service.get_event_analytics(
            event_id=event_id, days=days
        )
...
async def retrain_models(
    background_tasks: BackgroundTasks, admin_user: str = Depends(get_current_user)
):
...
async def get_model_status(admin_user: str = Depends(get_current_user)):
