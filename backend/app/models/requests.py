...
class UserPreferencesRequest(BaseModel):
    ...
    preferred_genres: List[str] = Field(default=[], description="Preferred music genres")
    ...
    price_range: tuple[float, float] = Field(default=(0, 1000), description="Price range (min, max) in DKK")
    preferred_times: List[int] = Field(default=[], description="Preferred hours (0-23)")
    preferred_days: List[int] = Field(default=[], description="Preferred days of week (0-6)")
    max_distance_km: float = Field(default=20.0, description="Maximum distance in kilometers")
...
class RecommendationRequest(BaseModel):
    ...
    use_collaborative: bool = Field(default=True, description="Use collaborative filtering")
    diversity_factor: float = Field(default=0.1, ge=0, le=1, description="Diversity promotion factor")
    explain: bool = Field(default=False, description="Include explanations for recommendations")
...
class InteractionRequest(BaseModel):
    ...
    rating: Optional[float] = Field(None, ge=1, le=5, description="Rating for 'went' interactions")
    source: Optional[str] = Field(None, description="Source of interaction (feed, search, etc.)")
    position: Optional[int] = Field(None, description="Position in recommendation list")
    
    @validator('rating')
    def validate_rating(cls, v, values):
        if 'interaction_type' in values and values['interaction_type'] == InteractionType.WENT:
            if v is None:
...
class EventSearchRequest(BaseModel):
    ...
    location_lat: Optional[float] = None
    location_lon: Optional[float] = None
    max_distance_km: float = Field(default=50.0, ge=0, description="Maximum distance from location")
    date_filter: Optional[DateFilter] = None
...
class FeedbackRequest(BaseModel):
    ...
    feedback_data: Dict[str, Any] = Field(default={}, description="Additional feedback data")
...
class ModelFeedbackRequest(BaseModel):
    ...
    session_id: str
    model_version: str
    feedback_score: float = Field(..., ge=0, le=1, description="Overall satisfaction (0-1)")
